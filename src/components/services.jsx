...
function MboileServices() {
   return( <div className="grid md:hidden">
        ...
        <MobileItemText title={"Graphic Design"} desc={"Great design makes you memorable. We deliver artwork that underscores your brand message and captures potential clients’ attention."} src= {"/images/mobile/image-graphic-design.jpg"}/>
        <MobileItemText title={"Photography"} desc={"Increase your credibility by getting the most stunning, high-quality photos that improve your business image."} src= {"/images/mobile/image-photography.jpg"}/>
    </div>)
}
...
function DesktopServices() {
   return( <div className="md:grid md:grid-cols-2 hidden ">
        ...
        <MobileTextService title={ "Stand out to the right audience"} desc={"Using a collaborative formula of designers, researchers, photographers, videographers, and copywriters, we’ll build and extend your brand in digital places."} link={"LEARN MORE"} linkClass={" fraunces-900 hover:underline"}/>
        <MobileItemText title={"Graphic Design"} desc={"Great design makes you memorable. We deliver artwork that underscores your brand message and captures potential clients’ attention."} src= {"/images/desktop/image-graphic-design.jpg"}/>
        <MobileItemText title={"Photography"} desc={"Increase your credibility by getting the most stunning, high-quality photos that improve your business image."} src= {"/images/desktop/image-photography.jpg"}/>
    </div>)
}
...
function MobileTextService({title,desc,link, linkClass}) {
    return(
        <div className="text-center p-12 space-y-6 md:space-y-2 lg:space-y-10">
            <h1 className="text-grey-950 text-3xl fraunces-900">{title}</h1>
            <p className="text-grey-55">{desc}</p>
            <a className={linkClass}>{link}</a>
            </div>
    )
}
function MobileItemText({title,desc,src}) {
    return(
